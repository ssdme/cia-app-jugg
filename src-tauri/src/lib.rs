...
pub fn create_plan_internal(
    ...
    for win in filtered_bounds.windows(2) {
        ...
        while seg_t0 < seg_t1 - 1e-9 {
            ...
            // Bouncy: 30%/15%/0%; when active, zero out harmonic a0 (unless custom_params explicitly set it)
            ...
            // If custom_params explicitly sets shake_a0, always honour it.
            // Otherwise apply the default rule: bouncy_shake suppresses harmonic a0.
...
#[tauri::command]
fn cmd_get_style_defaults(style: String) -> Result<CustomParams, String> {
...
mod tests {
    ...
    // ─── T18: Custom Params Tests ──────────────────────────────────────────────

    #[test]
    fn test_custom_params_override() {
        // custom shake_a0=99.0 must propagate to all segments (shakes enabled)
        ...
        for (i, seg) in plan.segments.iter().enumerate() {
            assert_eq!(
                seg.effects.shake.a0, 99.0,
                "Segment {} must have shake.a0 == 99.0, got {}",
                i, seg.effects.shake.a0
            );
        }
        println!("T18 custom_params_override: {} segments all have shake.a0=99.0", plan.segments.len());
    }

    #[test]
    fn test_toggle_priority_over_custom_params() {
        // shakes toggle=false overrides custom shake_a0=99.0 → all segments must have a0=0.0
        let beats: Vec<f64> = (0..16).map(|i| i as f64 * 0.72).collect();
        let downbeats = vec![2.88, 5.76, 8.64];
        ...
        for (i, seg) in plan.segments.iter().enumerate() {
            assert_eq!(
                seg.effects.shake.a0, 0.0,
                "Segment {} must have shake.a0==0.0 when shakes toggle=false, got {}",
                i, seg.effects.shake.a0
            );
        }
        println!("T18 toggle_priority: shakes=false overrides custom shake_a0=99.0 correctly");
    }
    ...
    fn test_get_style_defaults_values() {
        ...
        // HARD presets
        assert!(hard.shake_a0 > 0.0, "HARD shake_a0 must be > 0");
        assert!(hard.zoom_scale_start > 1.0 || hard.zoom_scale_end > 1.0,
            "HARD zoom must have some scale > 1.0");
        assert!(hard.flicker_amplitude > 0.0, "HARD flicker_amplitude must be > 0");
        assert!(hard.exposure_flash_peak > 0.0, "HARD exposure_flash_peak must be > 0");

        // SMOOTH presets must be gentler than HARD
        assert!(smooth.shake_a0 <= hard.shake_a0,
            "SMOOTH shake_a0 ({}) must be <= HARD ({})", smooth.shake_a0, hard.shake_a0);
        assert!(smooth.flicker_amplitude <= hard.flicker_amplitude,
            "SMOOTH flicker ({}) must be <= HARD ({})", smooth.flicker_amplitude, hard.flicker_amplitude);

        // HYBRID is between HARD and SMOOTH
        assert!(hybrid.shake_a0 >= smooth.shake_a0,
            "HYBRID shake_a0 ({}) must be >= SMOOTH ({})", hybrid.shake_a0, smooth.shake_a0);
        assert!(hybrid.shake_a0 <= hard.shake_a0,
            "HYBRID shake_a0 ({}) must be <= HARD ({})", hybrid.shake_a0, hard.shake_a0);

        // Basic sanity: all non-negative where applicable
